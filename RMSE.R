...
rmse_s1_df <- data.frame(
  ...
  rmse_fit = c(
    ...
    rmse(forecast_without_cov_auto[1:length(true_vals_train), "yhat"], true_vals_train)
    ),
  rmse_covid = c(
    ...
    rmse(forecast_without_cov_auto[(length(true_vals_train)+1):(N - length(true_vals_test)), "yhat"], true_vals_cov)
  ),
  rmse_test = c(
    ...
    rmse(forecast_without_cov_auto[(N- length(true_vals_test) + 1):N, "yhat"], true_vals_test)))
...
rmse_s2_df <- data.frame(
  ...
  rmse_fit = c(
    ...
    apply(sarima_models_fit$`z epidemią`[1:length(true_vals_train), ], 2, rmse, true_vals_train),
    rmse(forecast_with_cov_auto[1:length(true_vals_train), "yhat"], true_vals_train),
    rmse(forecast_with_cov[1:length(true_vals_train), "yhat"], true_vals_train)
    ),
  rmse_covid = matrix(NA, nrow = 5, ncol =1),
  rmse_test = c(
    rmse_table %>% filter(model_id == "211") %>% pull(rmse_pred),
    apply(sarima_models_fit$`z epidemią`[(N - length(true_vals_test) + 1):N, ], 2, rmse, true_vals_test),
    rmse(forecast_with_cov_auto[(N- length(true_vals_test) + 1):N, "yhat"], true_vals_test),
    rmse(forecast_with_cov[(N- length(true_vals_test) + 1):N, "yhat"], true_vals_test)))
...
rmse_s3_df <- data.frame(
  ...
  rmse_fit = c(
    ...
    apply(sarima_models_fit$imputacja[1:length(true_vals_train), ], 2, rmse, true_vals_train),
    rmse(forecast_imp_cov_auto[1:length(true_vals_train), "yhat"], true_vals_train)
    ),
  rmse_covid = matrix(NA, nrow = 4, ncol =1),
  rmse_test = c(
    rmse_table_imp %>% filter(model_id == "1101") %>% pull(rmse_pred),
    apply(sarima_models_fit$imputacja[(N - length(true_vals_test) + 1):N, ], 2, rmse, true_vals_test),
    rmse(forecast_imp_cov_auto[(N- length(true_vals_test) + 1):N, "yhat"], true_vals_test)))
