...
forecast_imp_cov_auto <- predict(prophet_models$imputacja,
...
rmse_s3_df <- data.frame(
  ...
  rmse_fit = c(
    ...
    rmse(forecast_imp_cov_auto[1:length(true_vals_train_with_cov), "yhat"], true_vals_train_with_cov_imp)
    ),
...
readRDS("rmse/rmse_results_df.RDS")

rmse_full_df %>% filter(scenario == 1) %>% sort_by(~rmse_fit)
rmse_full_df %>% filter(scenario == 1) %>% sort_by(~rmse_covid)
